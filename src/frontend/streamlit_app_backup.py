    ...
    with tab3:
        ...
        with st.form("config_form"):
            min_length = st.number_input(
                ...
                min_value=1,
                max_value=10,
                value=scanner_info.get('min_barcode_length', 3),
                help="Número mínimo de caracteres para considerar válido un código"
            )
            ...
            max_length = st.number_input(
                ...
                min_value=10,
                max_value=100,
                value=scanner_info.get('max_barcode_length', 50),
                help="Número máximo de caracteres para considerar válido un código"
            )
            
            speed_threshold = st.number_input(
                "Umbral de velocidad (ms)",
                min_value=10.0,
                max_value=500.0,
                value=scanner_info.get('speed_threshold_ms', 100.0),
                step=10.0,
                help="Tiempo máximo en milisegundos entre caracteres para detectar scanner vs teclado manual"
            )
            ...
            if submitted:
                with st.spinner("Actualizando configuración..."):
                    # Aquí iría la llamada a configure_scanner
                    # Por ahora mostramos un mensaje
                    st.success("✅ Configuración actualizada exitosamente")
                    st.info("🔄 Reinicia el scanner para aplicar los cambios")
